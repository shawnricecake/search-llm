...
class OPTAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k_proj = Linear_Super(embed_dim, embed_dim, bias=bias, num_head=self.num_heads, head_dim=self.head_dim)
        self.v_proj = Linear_Super(embed_dim, embed_dim, bias=bias, num_head=self.num_heads, head_dim=self.head_dim)
        self.q_proj = Linear_Super(embed_dim, embed_dim, bias=bias, num_head=self.num_heads, head_dim=self.head_dim)
        self.out_proj = Linear_Super(embed_dim, embed_dim, bias=bias)
    ...
    def set_sample_config(self, sample_hidden_size, sample_num_attention_heads, sample_weight_start):
        ...
        self.q_proj.set_sample_config(
            self.embed_dim, self.sample_num_heads * self.sample_head_dim, sample_weight_start,
        ...
        self.out_proj.set_sample_config(self.sample_hidden_size, self.embed_dim, sample_weight_start)
    ...
    def forward(
        ...
        proj_shape = (bsz * self.sample_num_heads, -1, self.sample_head_dim) \
            if self.sample_num_heads and self.sample_head_dim \
            else (bsz * self.num_heads, -1, self.head_dim)
        ...
        if attn_weights.size() != (
                bsz * (self.sample_num_heads if self.sample_num_heads else self.num_heads),
                tgt_len, src_len
        ):
            raise ValueError(
                f"Attention weights should be of size "
                f"{(bsz * (self.sample_num_heads if self.sample_num_heads else self.num_heads), tgt_len, src_len)}, "
                f"but is"
                f" {attn_weights.size()}"
            )
        ...
        if layer_head_mask is not None:
            if layer_head_mask.size() != (self.sample_num_heads if self.sample_num_heads else self.num_heads, ):
                raise ValueError(
                    f"Head mask for a single layer should be of size "
                    f"{(self.sample_num_heads if self.sample_num_heads else self.num_heads,)}"
                    f", but is"
        ...
        if attn_output.size() != (
            ...
            raise ValueError(
                ...
                f"{(bsz, self.sample_num_heads if self.sample_num_heads else self.num_heads, tgt_len, self.sample_head_dim if self.sample_head_dim else self.head_dim)}"
                f", but is"
                f" {attn_output.size()}"
            )
        ...
        attn_output = attn_output.reshape(
            bsz, tgt_len, self.sample_hidden_size if self.sample_hidden_size else self.embed_dim
        )
...
class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig):
        ...
        self.fc1 = Linear_Super(self.embed_dim, config.ffn_dim, bias=config.enable_bias)
        self.fc2 = Linear_Super(config.ffn_dim, self.embed_dim, bias=config.enable_bias)
    ...
    def set_sample_config(
        ...
        self.fc1.set_sample_config(self.embed_dim, sample_intermediate_size, sample_weight_start)
        self.fc2.set_sample_config(sample_intermediate_size, self.embed_dim, sample_weight_start)
...
class OPTDecoder(OPTPreTrainedModel):
    ...
    def __init__(self, config: OPTConfig):
        ...
        self.embed_tokens = Embedding_Super(config.vocab_size, config.word_embed_proj_dim, self.padding_idx)

        self.embed_positions = OPTLearnedPositionalEmbedding(config.max_position_embeddings, config.hidden_size)

        if config.word_embed_proj_dim != config.hidden_size:
            # self.project_out = nn.Linear(config.hidden_size, config.word_embed_proj_dim, bias=False)
            self.project_out = Linear_Super(config.hidden_size, config.word_embed_proj_dim, bias=False)
        else:
            self.project_out = None

        if config.word_embed_proj_dim != config.hidden_size:
            # self.project_in = nn.Linear(config.word_embed_proj_dim, config.hidden_size, bias=False)
            self.project_in = Linear_Super(config.word_embed_proj_dim, config.hidden_size, bias=False)
        else:
...
class OPTModel(OPTPreTrainedModel):
    ...
    def set_sample_config(self, sample_config: dict):
        self.decoder.set_sample_config(sample_config)
...
class OPTForCausalLM(OPTPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.lm_head = Linear_Super(config.word_embed_proj_dim, config.vocab_size, bias=False)
    ...
    def set_sample_config(self, sample_config: dict):
        self.model.set_sample_config(sample_config)

    def get_sampled_params_numel(self, sample_config: dict):
        self.set_sample_config(sample_config)
        numels = []
        for name, module in self.model.named_modules():
            if hasattr(module, 'calc_sampled_param_num'):
                if name.split('.')[0] == 'layers' and int(name.split('.')[1]) >= sample_config['sample_layer_num']:
                    continue
        ...
        return sum(numels) + self.lm_head.calc_sampled_param_num()
